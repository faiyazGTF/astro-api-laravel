...
class HomePageController extends CommonController
{
    ...
    public function SimilarAstrologer(Request $request)
    {
        ...
        $type = $request->input('type'); //type=call,chat,video lke this or can be call ,chat,both
        $types = array_filter(explode(',', $type));
        if (!empty($types)) {
            $conditions = [];
            foreach ($types as $t) {
                $conditions[] = "FIND_IN_SET('$t', users_details.flags)";
            }
        ...
        $astro = DB::table('users')
            ...
            ->where('users_details.availability', 1)
