...
Route::any('mtstatus', function (Request $request) {
    
            //   DB::select("INSERT INTO `exotels` (`data`,`session_id`) VALUES ('fz','faiz_api_webshooks')");
 ...
 DB::select("INSERT INTO `exotels` (`data`,`session_id`) VALUES ('$data','$session_id')");
        ...
        if ($status == 'no-answer') {
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => 'https://app.astroera.in/api/user/chat/end',
        ...
        if ($ongoingstatus == 'in-progress') {
            ...
            curl_setopt_array($curl, array(
				...
				CURLOPT_URL => 'https://app.astroera.in/api/astrologer/accept-session/' . $session_id . '?StartTime=' . urlencode($StartTime) . '&EventTime=' . urlencode($EventTime),
